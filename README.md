...
`dist/data/screenings.json` contains the last collected snapshot, per-venue status, actual successful-fetch timestamps, official program URLs, and festivals. The footer strip carries festivals only, not a venue's curated series: entries are kept when their title matches the named-festival list in `FESTIVAL`, so NYFF and its like appear while a Metrograph retrospective does not. Between festival seasons an empty strip is a valid result rather than a failure. Sources are aggregated across venues through `SERIES_SOURCES` — Metrograph and Film at Lincoln Center today, one parser each — and a source that fails keeps its previous entries rather than emptying the strip. Lincoln Center hydrates its calendar from a JSON payload carrying escaped markup, so that parser unescapes before reading; it keys on link path and heading rather than the utility classes around them. Festivals from a venue with no filter chip stay visible whatever chips are selected. The scraper uses public HTML or the public endpoint used by each venue's site; it does not bypass access controls.
